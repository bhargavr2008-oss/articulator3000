...
- `POST /api/sessions`, server-only OpenAI client, env wiring (Vercel Postgres + Blob).
...
- Question UI: spoken (browser speech synth) + visible; answer by voice or type; the four
  escape hatches ("Why asking?" / "Don't know" / "Skip" / "Wrong assumption").
...
- "Explain it to…" — teammate / investor / coding-agent. **On-demand per click**, rephrasing
  one-liner + summary. "Copy for chat" copies the tailored blurb + link.
...
- **Share-card image:** composite hero + reconstructed sketch + title into one graphic; set as
  the page's `og:image`. (Generate with canvas/`@vercel/og`, or compose client-side and upload
  to Blob.)
- Server-rendered share page with Open Graph tags + `noindex`. Manual **delete now** button +
  "temporary link" copy. **No expiry cron.**
- **"Copy for chat"**: copies 2–3 sentence blurb + link (unfurls into the share card on paste).
...
1. Audience switcher → ship 1 audience instead of 3.
